**Assign Cookies**: ([link](https://leetcode.com/problems/assign-cookies/)) sort both and then compare from left to right (or right to left). The idea is to assign each cookie to a child with min possible greed that satisfies it. If we assign cookie `1` to `4` in `2 3 4` instead of `2`, we won't be able to assign cookie `4` to any child, so that won't be optimal.
