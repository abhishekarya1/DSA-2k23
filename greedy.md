...
**Candy**: ([link](https://leetcode.com/problems/candy/)) fairly easy using two passes and `O(N)` space by looking at only left / right neighbours and then choosing maximum of each. The linear solution using slope approach is cracked! watch Striver's video.
