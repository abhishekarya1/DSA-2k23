...
**Job Scheduling** with Deadlines and Profit: simply picking max profits first won't work e.g. `{ (1, 10), (2, 50) }` has max profit as `60` and not `50` (if we pick `50` for time unit `1`). Sort jobs in desc order of profit, then dump each job into the last free slot before its deadline, by doing reverse traversal on a `slots[maxDeadline + 1]` array. ([explanation](https://chatgpt.com/share/6938610b-79c8-800e-afb2-56c86046d753))
