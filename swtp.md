...
- [Minimum Length of String After Deleting Similar Ends](https://leetcode.com/problems/minimum-length-of-string-after-deleting-similar-ends/) - on same `i` and `j` char, remove same chars repeatedly from each end. Stop at a single element `i == j` to calc length as `j - i + 1`. Cases to test: `aabca` (normal case), `a` (single char), `aaaa` (i crosses j in execution)
