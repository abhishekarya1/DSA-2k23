...
- [Container With Most Water](https://leetcode.com/problems/container-with-most-water): sort of like Trapping Rainwater but such complicated two-pointers isn't needed here. Simple two pointers will work because we're focusing on only two bars at a time and not trapping rainwater falling from above in the whole histogram. Water between two pointer, track max of `ans = max(ans, height[high] * (high - low))`
