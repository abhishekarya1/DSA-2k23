...
**INSIGHT**: BFS progresses for all starting points in order one step at a time! All starting points are executed first (FIFO) and then their respective neighbours are added to queue. Therefore we can't have cases where one point traverses lots of steps (say 4) and store it in corresponding `dist[][]` cell while a `0` cell is its immediate neighbour in the above problem.
