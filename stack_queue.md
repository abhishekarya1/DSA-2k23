...
**Infix to Prefix**: reverse given infix expression, then make it "nearly-postfix" by converting `(` to `)` and vice-versa, then convert it using `infixToPostfix()` and store in `resultStr`, reverse `resultStr`.
