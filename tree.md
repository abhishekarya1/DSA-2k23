...
**Maximum Width of BT**: ([link](https://leetcode.com/problems/maximum-width-of-binary-tree/)) use BFS and consider index of nodes as `2*curr + 1` for left and `2*curr + 2` for right. For each level, check min present index `i = 0` and max present index (`i = n-1`), and calc and track max width as `right - left + 1`.
