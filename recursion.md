...
**Problems**:
- [Combination Sum](https://leetcode.com/problems/combination-sum/): all distinct elements, allowed to pick same element multiple times, sorting is optional but it can help in optimization that we can prune not-pick step too if `arr[i] > target`.
