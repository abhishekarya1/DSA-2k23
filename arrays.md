...
- `O(n)` time, `O(n)` space hashmap/unordered_map approach or use set (still needs `O(n)` space)
- `O(n)` time, `O(1)` space approaches (mark indices approach)
	- multiply with `-1` (works only with +ve elements)
	- cycle sort swapping (must do 1-based indexing to fit all elements)
